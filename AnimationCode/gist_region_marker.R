...
GeoAnimation.Region.Marker = function(qdata,showyear) {
  week = qdata$week
  year = qdata$year
  ...
  total.region.std = as.data.frame(apply(total.region,2,normalization))
  ...
  total.region.std = cbind.data.frame(region.name,LatLong, t(total.region.std))
  colnames(total.region.std) = c("Region","LatLong",seq(1,length(year.index),1))
  long.total.region = gather(total.region.std,date,value,3:ncol(total.region.std))
  long.total.region$size = long.total.region$value * 50
  ...
  gvisData <- by(long.total.region, list(date=long.total.region$date), function(x){
    date <- x$date[1]	
    g <- gvisGeoChart(x,  locationvar = "LatLong", colorvar = "value", sizevar = "size", hovervar = "Region",
                      options=list(region="US", displayMode="markers",
                                   sizeAxis.minSize = 10, sizeAxis.maxSize = 100,
                                   sizeAxis.minValue = 0, sizeAxis.maxValue = 50),
  ...
  gvisChart <- sprintf('
                       ...
                       options["sizeAxis.minSize"] = 10;
                       options["sizeAxis.maxSize"] = 100;
                       options["sizeAxis.minValue"] = 0;
                       options["sizeAxis.maxValue"] = 50;
                       options["width"] =   600;
                       options["height"] =  600;
...
plot(GeoAnimation.Region.Marker(qdata,showyear = 2016))
